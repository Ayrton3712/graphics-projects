...
int main() {
	...
	std::vector<Point> points = midpoint_circle(0.0f, 0.0f, 500.0f);
	
	// Normalizing points to fit within the NDC range of [-1, 1]
	for (size_t i = 0; i < points.size(); i++) {
		points[i].normalizePoint(1000.0f, 700.5f);
	}

	// Sort points by angle so outlines/triangle fans connect correctly
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Midpoint Circle", nullptr, nullptr);
	if (window == nullptr) {
